...
scrublet <- function(object, min_counts=2, min_cells=3, expected_doublet_rate=0.06, min_gene_variability_pctl=85, n_prin_comps=50, sim_doublet_ratio=2, n_neighbors=NULL) {
    ...
    X <- suppressWarnings(as(t(as.matrix(LayerData(object=object, slot='counts'))), 'TsparseMatrix'))
    ...
    doublet_scores <- scrublet_res$doublet_scores

    names(doublet_scores) <- colnames(object)
